// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'aiuta_icons.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

AiutaIcons _$AiutaIconsFromJson(Map<String, dynamic> json) => AiutaIcons(
      preonboarding82:
          AiutaIcon.fromJson(json['preonboarding82'] as Map<String, dynamic>),
      error36: AiutaIcon.fromJson(json['error36'] as Map<String, dynamic>),
      like36: AiutaIcon.fromJson(json['like36'] as Map<String, dynamic>),
      dislike36: AiutaIcon.fromJson(json['dislike36'] as Map<String, dynamic>),
      imageError36:
          AiutaIcon.fromJson(json['imageError36'] as Map<String, dynamic>),
      back24: AiutaIcon.fromJson(json['back24'] as Map<String, dynamic>),
      camera24: AiutaIcon.fromJson(json['camera24'] as Map<String, dynamic>),
      checkCorrect24:
          AiutaIcon.fromJson(json['checkCorrect24'] as Map<String, dynamic>),
      checkNotCorrect24:
          AiutaIcon.fromJson(json['checkNotCorrect24'] as Map<String, dynamic>),
      close24: AiutaIcon.fromJson(json['close24'] as Map<String, dynamic>),
      trash24: AiutaIcon.fromJson(json['trash24'] as Map<String, dynamic>),
      takePhoto24:
          AiutaIcon.fromJson(json['takePhoto24'] as Map<String, dynamic>),
      history24: AiutaIcon.fromJson(json['history24'] as Map<String, dynamic>),
      photoLibrary24:
          AiutaIcon.fromJson(json['photoLibrary24'] as Map<String, dynamic>),
      share24: AiutaIcon.fromJson(json['share24'] as Map<String, dynamic>),
      wishlist24:
          AiutaIcon.fromJson(json['wishlist24'] as Map<String, dynamic>),
      wishlistFill24:
          AiutaIcon.fromJson(json['wishlistFill24'] as Map<String, dynamic>),
      check20: AiutaIcon.fromJson(json['check20'] as Map<String, dynamic>),
      info20: AiutaIcon.fromJson(json['info20'] as Map<String, dynamic>),
      magic20: AiutaIcon.fromJson(json['magic20'] as Map<String, dynamic>),
      lock16: AiutaIcon.fromJson(json['lock16'] as Map<String, dynamic>),
      arrow16: AiutaIcon.fromJson(json['arrow16'] as Map<String, dynamic>),
      loading14: AiutaIcon.fromJson(json['loading14'] as Map<String, dynamic>),
    );

Map<String, dynamic> _$AiutaIconsToJson(AiutaIcons instance) =>
    <String, dynamic>{
      'preonboarding82': instance.preonboarding82,
      'error36': instance.error36,
      'like36': instance.like36,
      'dislike36': instance.dislike36,
      'imageError36': instance.imageError36,
      'back24': instance.back24,
      'camera24': instance.camera24,
      'checkCorrect24': instance.checkCorrect24,
      'checkNotCorrect24': instance.checkNotCorrect24,
      'close24': instance.close24,
      'trash24': instance.trash24,
      'takePhoto24': instance.takePhoto24,
      'history24': instance.history24,
      'photoLibrary24': instance.photoLibrary24,
      'share24': instance.share24,
      'wishlist24': instance.wishlist24,
      'wishlistFill24': instance.wishlistFill24,
      'check20': instance.check20,
      'info20': instance.info20,
      'magic20': instance.magic20,
      'lock16': instance.lock16,
      'arrow16': instance.arrow16,
      'loading14': instance.loading14,
    };
